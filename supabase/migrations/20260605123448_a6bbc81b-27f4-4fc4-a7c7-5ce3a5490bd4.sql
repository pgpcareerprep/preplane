-- Gate the LMP sheet-mirror trigger so the `comments` column is only pushed
-- back to Sheets column Z when the change came from the app (post_lmp_comment).
-- For sync_source in ('csv_import','copilot','voice-copilot','system', etc.)
-- we skip the comments push entirely so stale text cannot clobber chat history.
...
BEGIN
  ...
  -- Only include `comments` in the patch when the change originated from
  -- the app (post_lmp_comment). CSV import, copilot, voice copilot, system
  -- writes etc. must NEVER push the `comments` column to Sheets column Z,
  -- otherwise stale text would clobber the real chat history.
  IF COALESCE(NEW.sync_source, '') = 'app'
     AND NEW.comments IS DISTINCT FROM OLD.comments THEN
    v_db_patch := v_db_patch || jsonb_build_object('comments', NEW.comments);
  END IF;
