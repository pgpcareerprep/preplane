...
SELECT id, company, role, domain_raw, domain_id, status, type,
    ...
    (SELECT count(*) FROM lmp_candidates c WHERE c.lmp_id = l.id AND lower(COALESCE(c.pipeline_stage,'')) = ANY (ARRAY['r1','r1_shortlisted','shortlisted','round_1','round1'])) AS r1_count,
    (SELECT count(*) FROM lmp_candidates c WHERE c.lmp_id = l.id AND lower(COALESCE(c.pipeline_stage,'')) = ANY (ARRAY['r2','r2_shortlisted','round_2','round2'])) AS r2_count,
    (SELECT count(*) FROM lmp_candidates c WHERE c.lmp_id = l.id AND lower(COALESCE(c.pipeline_stage,'')) = ANY (ARRAY['r3','r3_shortlisted','round_3','round3'])) AS r3_count,
    (SELECT count(*) FROM lmp_candidates c WHERE c.lmp_id = l.id AND lower(COALESCE(c.pipeline_stage,'')) = ANY (ARRAY['offer','converted','final','accepted'])) AS offer_count,
    ...
    COALESCE((SELECT lm.feedback_avg FROM lmp_mentors lm WHERE lm.lmp_id = l.id AND lm.status = 'assigned' ORDER BY lm.assigned_at DESC LIMIT 1), (SELECT avg(s.mentor_rating) FROM sessions s WHERE s.lmp_id = l.id AND s.mentor_rating IS NOT NULL)) AS mentor_feedback_avg,
